...
static
bool
isBaseMatchForPoorAlignmentTest(
    const char a,
    const char b)
{
    ...
    pos_t& leadingRefPos)
{
    ...
    for (const path_segment& ps : bamAlign.path)
    {
        if (is_segment_align_match(ps.type))
        {
            ...
            {
                if (isBaseMatchForPoorAlignmentTest(querySeq.get_char(readIndex + segPos),
                                                    refSeq.get_base(refIndex + segPos)))
                {
                    matchLength++;
...
/// a continuous block of \p contiguousMatchCount basecall matches between the read and the reference sequence.
///
/// Example:
/// ```
...
/// ```
///
/// \param[in] contiguousMatchCount Minimum length of contiguous matching block used to define edge mismatch length.
/// \param[out] leadingLength Length in read coordinates from the start of the read to the base preceding the first matching segment.
/// \param[out] leadingRefPos Reference position of read start after removing leadingLength from the beginning of the read.
/// \param[out] trailingLength Length in read coordinates from the end of the read to the base following the last matching segment.
/// \param[out] trailingRefPos Reference position of read end after removing trailingLength from the end of the read.
///
static
void
edgePoorAlignmentLength(
    ...
    pos_t& trailingRefPos)
{
    leadingEdgePoorAlignmentLength(bamAlign, querySeq, refSeq, contiguousMatchCount, leadingLength, leadingRefPos);
    trailingEdgePoorAlignmentLength(bamAlign, querySeq, refSeq, contiguousMatchCount, trailingLength, trailingRefPos);
    ...
    const float minHighBasecallQualityFraction)
{
    ...
    edgePoorAlignmentLength(matchedAlignment, querySeq, refSeq, contiguousMatchCount,
                            leadingEdgePoorAlignmentLengthTmp, leadingEdgeRefPos,
                            trailingEdgePoorAlignmentLengthTmp, trailingEdgeRefPos);
    ...
    if (0 != leadingEdgePoorAlignmentLengthTmp)
    {
        if ((! isOverlappingReadPair) || bamRead.isSASplit() || bamRead.is_fwd_strand())
        {
            ...
            if ((static_cast<float>(highQualityCount)/(leadingEdgePoorAlignmentLengthTmp)) >= minHighBasecallQualityFraction)
            {
        ...
        {
            log_os << " Overlapping_pair leading" << " read qname=" << bamRead.qname() << "\n";
        }
#endif
    }
    ...
    if (0 != trailingEdgePoorAlignmentLengthTmp)
    {
        if ((! isOverlappingReadPair) || bamRead.isSASplit() || (! bamRead.is_fwd_strand()))
        {
            ...
            if ((static_cast<float>(highQualityCount)/(trailingEdgePoorAlignmentLengthTmp)) >= minHighBasecallQualityFraction)
            {
        ...
        {
            log_os << "Overlapping_pair trailing" << " read qname=" << bamRead.qname() << "\n";
        }
#endif
    }
}
