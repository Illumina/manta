    ...
    boost::array<double,DIPLOID_GT::SIZE>& loglhood)
{
    BOOST_FOREACH(const SVEvidence::evidenceTrack_t::value_type& val, sampleEvidence)
    {
        ...
        if (! getRefAltFromFrag(spanningPairWeight, semiMappedPower, chimeraProb, refSplitMapProb, altSplitMapProb, isPermissive, fragLabel, fragev,
    ...
    SVScoreInfoSomatic& somaticInfo)
{
    ...
    for (unsigned tierIndex(0); tierIndex<tierCount; ++tierIndex)
    {
        ...
        BOOST_FOREACH(const JunctionCallInfo& junction, junctionData)
        {
            ...
            const float& spanningPairWeight(junction.getSpanningWeight());
            computeSomaticSampleLoghood(spanningPairWeight, evidence.tumor, somaticMutationFreq, noiseMutationFreq, isPermissive,
            ...
            computeSomaticSampleLoghood(spanningPairWeight, evidence.normal, 0, noiseMutationFreq, isPermissive,
