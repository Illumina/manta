...
private:
    ...
    /// the pointer edge and all previous edges is greater than _beginCount. _headCount will be updated to reflect
    /// the above sum
    ///
    /// In the case that all edges where _headCount is greater than _beginCount are filtered, the function will set
    /// _headCount to _endCount+1 and _edge to an undefined state.
    ///
    void
    jumpToFirstEdge();
    ...
    /// Provide the observation range for the bin we're retrieving, these values should be constant following the ctor
